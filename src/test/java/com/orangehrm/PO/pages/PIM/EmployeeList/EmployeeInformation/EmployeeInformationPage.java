    ...
    public void deleteAllEmployeesFromPIM() {
        while (isElementPresent(employeeInformationLocators.allEmployeesListInTable)) {
            if (employeeInformationLocators.allEmployeesListInTable.size() > 1) {
                ...
                logInfo("Only 1 employee present, cannot be deleted further", true);
                break;
            }
        }
    }

    public void validateNoEmployeesPresentInTable() {
        if (employeeInformationLocators.allEmployeesListInTable.size() > 1) {
            System.out.println("Number of employees in the table: " + employeeInformationLocators.allEmployeesListInTable.size());
            logInfo("No employees present in the employees table", true);
        } else {
            logFail("Employees are still present in the system", true);
        }
    }
}
